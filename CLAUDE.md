...
3. **테스트 단정 갱신** (CI 빨갛게 되는 흔한 세 군데)
   ...
   - **유일하게 한 KC 를 커버하던 과제를 교체할 때** — 같은 파일의 "CS1 필수 KC 10개를 모두 최소 1회 커버" 테스트의 `required` 배열도 갱신. 예: A09 가 `recursion` 단독 커버였는데 파일 입출력으로 교체 → `required` 에서 `recursion` → `file-io` 치환. 안 하면 커버리지 테스트가 빨개짐. 새 KC 는 `KC_OBJECTIVES` 맵에도 학습목표 문장을 추가해야 학생 화면·챗봇 시작 대화에 노출됨 (`getLearningObjectives(kcTags)` 가 양쪽 공통 경로).
...
| 2026-06-09 | A09·A10 카탈로그 교체 (둘 다 `code` 슬러그 유지·version 1→2, `UNLOCKED_PREFIXES` 추가 → A00~A10 전부 공개). **A09**: 재귀 팩토리얼 → 파일 입출력(fopen/fprintf/fgets/fclose). Judge0 서버 백엔드는 실제 파일시스템이라 단일 실행 내 파일 I/O 동작 확인(clang-wasm 워커는 placeholder). 신규 KC `file-io`·`file-error-handling` + 학습목표 2개. **recursion 단독 과제였으므로** assignments.test.ts 커버리지 `required` 배열 `recursion`→`file-io` 치환. **A10**: printf 구구단 → 구조체 배열+함수 성적 처리(printStudents/getAverage/printAboveAverage 함수 원형을 starter 에 제시, 본문만 TODO 1~3), 제목 "종합문제", kcTags structs-basic/functions-params/control-flow-loop. | `packages/db/src/seeds/assignments.ts`, `apps/student/components/AssignmentPanel.tsx`, `supabase/seed*` | 커리큘럼 파일 입출력·종합 문항 추가 + 플레이북 §1-3 KC 커버리지 함정 명문화 |
| 2026-06-09 | "내 학습 기록" 개선 — (1) `/api/my/submissions` 가 `code`·`reflection` 반환 + **통과한 과제만** `referenceSolutions` 맵 동봉(미통과 정답 노출 차단 = Safety Guard 원칙). (2) MyLearningDialog 제출 항목 펼치면 제출 코드·성찰일지(Q1/Q3/Q5)·정답 코드(통과 시) 표시. (3) "과제별 최고 수준" 섹션 제거. (4) "최근 제출" → `assignmentCode` 별 최신 1건만(재제출 중복 제거) 최대 10개, 제목 "과제별 최근 제출". A09 학습목표 ② `io-formatting`("printf·scanf…") 이 파일 과제에 겉돌아 `file-error-handling`("파일 열기 실패 시 오류 메시지 출력")으로 교체. | `apps/student/app/api/my/submissions/route.ts`, `apps/student/components/{MyLearningDialog,StudentWorkspace}.tsx`, `packages/db/src/seeds/assignments.ts` | 학생 자기 회고 도구 강화 + 정답 유출 게이팅 |
